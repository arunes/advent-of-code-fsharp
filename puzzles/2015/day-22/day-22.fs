module Puzzles.Year2015.Day22
// solution taken from: https://gist.github.com/latkin/45d8e009f471b4b5d609
type Spell =
    { Name: string
      Cost: int
      Damage: int
            ...
            if bossHp <= 0 then
                ...
                if myTurn then
                    ...
                    match
                        allSpells
                        |> List.filter (fun s ->
                            (s.Cost <= mana) && not (spells |> List.exists (fun s' -> s.Name = s'.Name)))
                    with
                    | [] -> best
                    | buyableSpells ->
                        ...
                        for s in buyableSpells do
                            ...
                            if bossHp <= 0 then
                                newBest <- min newBest spent
                            else
                                newBest <-
                                    min newBest (play part false newBest spent (hp, mana, spells) (bossHp, bossDamage))
